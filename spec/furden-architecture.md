...
- ~~**Subscriber-aware creator profile.**~~ DONE 2026-07-02. `/$handle` now runs an authenticated single-creator feed for signed-in viewers: `enumerateSubscriptions` gained an optional `creatorProxy` filter (both topics are indexed — one narrowed getLogs), active held tiers feed the new shared `loadTierContent(creator, tier)` (extracted from `assembleFeed`, which now reuses it), and unlocked posts merge with public posts newest-first, deduped by fingerprint. Failure degrades to the public view with a muted note, never a broken page.

Also closed in the same pass (Phase C polish):
